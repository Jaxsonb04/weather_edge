...
def test_sleeve_capacity_evidence_target_hand_computed() -> None:
    ...
    # Active ROI v4: reference_equity=1000, max_aggregate_risk_pct=0.25.
    assert evidence.capacity_dollars == pytest.approx(250.0)
    assert evidence.window_total_dollars_at_risk == pytest.approx(100.0)
    assert evidence.window_total_utilization_pct == pytest.approx(0.40)
...
def test_capacity_evidence_window_total_overstates_versus_max_daily_concurrency_figure() -> None:
    ...
    # Active ROI v4 capacity_dollars = 1000 * 0.25 = $250.
    assert evidence["target"].window_total_utilization_pct == pytest.approx(1.08)
    assert evidence["target"].max_daily_utilization_pct == pytest.approx(0.036)
