...
def test_target_clips_to_correlated_region_scenario_room() -> None:
    ...
    active = [
        # 2 x $170 = $340 pending leaves $20 of the v6 $360 west-coast region cap.
        pending_leg("KXHIGHTSEA", "2026-07-20", 170.0),
        pending_leg("KXHIGHLAX", "2026-07-20", 170.0),
    ]
    candidates = [
        ResearchOpportunity(
            _decision(
...
def test_target_blocks_when_city_target_room_cannot_fund_one_contract() -> None:
    ...
    active_decision = _decision(
        market,
        side="YES",
        # v6 city cap is $180; leave under one $0.50 contract of room.
        spend=179.6,
        probability=0.70,
        edge=0.20,
        edge_lcb=0.10,
...
def test_target_priority_uses_position_capped_quantity_before_log_growth() -> None:
    ...
    ordinary = ResearchOpportunity(
        _decision(
            _market(
                "72° to 73°",
                ticker="KXHIGHTSFO-26JUL20-B72.5",
                floor=72,
                cap=73,
                yes_ask=0.20,
            ),
...
def test_target_clips_to_aggregate_open_scenario_room() -> None:
    ...
    for idx in range(8):
        ...
        decision = _decision(
            market,
            side="YES",
            # 8 x $92.50 = $740 pending leaves $10 of the v6 $750 aggregate cap.
            spend=92.5,
            probability=0.70,
            edge=0.20,
            edge_lcb=0.10,
