...
def _fresh_root(parent: Path, *, operational_minutes: int = 1, strategy_minutes: int = 1) -> Path:
...
def test_watchdog_optionally_rejects_stale_public_manifest():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        payload["published_at"] = _iso(stale)
        payload["artifacts"]["trading_signal.json"]["generated_at"] = _iso(stale)
